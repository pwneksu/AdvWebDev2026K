...
function buildValidationMessage(errors) {
  ...
  return `Validation failed! Some fields are incorrect. Please fix the errors below and try again:\n\n${lines.join("\n")}`;
}
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 409) {
        ...
        const msg = `Resource already exists! A resource with the name "${name}" is already in the system. Please try a different name.`;
    ...
    const msgLines = [];
    msgLines.push("Success! The resource was added to the database.\n");
